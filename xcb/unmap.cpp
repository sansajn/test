// unmapping window (so it becomes hidden)
#include <chrono>
#include <thread>
#include <iostream>
#include <stdlib.h>
...
int main ()
{
	...
	while (true)
	{
		...
		if (t > 5s && t < 10s && visible)  // hide window
		{
			cout << "hide_window()" << endl;
			xcb_unmap_window(connection, window);
			xcb_flush(connection);
			visible = false;
		}
		else if (t > 10s && !visible)  // show window again
		{
			cout << "show_window()" << endl;
			xcb_map_window(connection, window);
			xcb_flush(connection);
			visible = true;
		}
