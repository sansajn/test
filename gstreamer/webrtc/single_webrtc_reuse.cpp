/* Single client WebRTC (h264) stream server with pipeline reuse sample. 
Re-negotiation doesn't seems to be possible at least in 1.16 GStreamer version. */
#include <string>
#include <thread>
#include <chrono>
#include <string_view>
...
void reuse_receiver_entry(ReceiverEntry * receiver, SoupWebsocketConnection * connection);
...
void new_client_request(SoupWebsocketConnection * connection) {
	...
	if (!pipeline) {
		...
		reuse_receiver_entry(pipeline, connection);
	}
}
...
void websocket_closed_handler(SoupWebsocketConnection * connection, gpointer user_data) {
	...
	// we just want to close a connection there and put pipeline into NULL state
	// destroy_receiver_entry(pipeline);
	// pipeline = nullptr;

	// TODO: this is ugly hack we need proper solution there
	if (pipeline->connection != nullptr) {
		if (pipeline->pipeline != nullptr)  // stop pipeline
			gst_element_set_state(GST_ELEMENT(pipeline->pipeline), GST_STATE_NULL);

		g_object_unref(G_OBJECT(pipeline->connection));
		pipeline->connection = nullptr;
	}

	//quit_loop();  // TODO; just for a valgrind output, quit loop on client close
}
...
void reuse_receiver_entry(ReceiverEntry * receiver, SoupWebsocketConnection * connection) {
	assert(receiver->connection == nullptr && "connection is expected to be already closed");

	receiver->connection = connection;
	g_object_ref(G_OBJECT(connection));

	g_signal_connect(G_OBJECT(connection), "message",
		G_CALLBACK(soup_websocket_message_cb), (gpointer)receiver);

	GArray * transceivers;
	g_signal_emit_by_name(receiver->webrtcbin, "get-transceivers",
		&transceivers);
	g_assert(transceivers != nullptr && transceivers->len > 0);
	...
	g_signal_connect(receiver->webrtcbin, "on-negotiation-needed",
		G_CALLBACK(on_negotiation_needed_cb), (gpointer)receiver);

	g_signal_connect(receiver->webrtcbin, "on-ice-candidate",
		G_CALLBACK(on_ice_candidate_cb), (gpointer)receiver);

	spdlog::info("pipeline reused");
...
void soup_websocket_message_cb(G_GNUC_UNUSED SoupWebsocketConnection * connection,
	...
	assert(connection == pipeline->connection && "only one opened connection is expected");
