/*! thread context h264 WebRTC streamer with video re-encoding.
h264streamer sample with thread local context. */
#include <string>
#include <iostream>
#include <locale.h>
#include <glib.h>
...
using std::string, std::cout, std::endl, std::signal;
...
main (int argc, char *argv[])
{
  ...
  assert(g_main_context_is_owner(ctx));
...
//  g_unix_signal_add (SIGSEGV, sigsegv_sighandler, nullptr);
  signal(SIGSEGV|SIGFPE, crash_handler);
#endif
  ...
  g_main_loop_run (mainloop);  // blocking
