...
static const gchar *file_name = "test.mp4";
...
static GOptionEntry entries[] = {
  ...
  {"file", 0, 0, G_OPTION_ARG_STRING, &file_name,
      "Streaming source (video file) URI (like 'file:///home/me/test.mp4')", "URI"},
  {NULL},
};
...
start_pipeline (void)
{
  ...
  char const * format = "webrtcbin bundle-policy=max-bundle name=sendrecv "
      STUN_SERVER
      "uridecodebin uri=%s ! videoconvert ! queue ! vp8enc deadline=1 ! rtpvp8pay ! "
      "queue ! " RTP_CAPS_VP8 "96 ! sendrecv. ";

  char pipe1_desc[4096] = {0};
  sprintf(pipe1_desc, format, file_name);

  pipe1 = gst_parse_launch (pipe1_desc, &error);
    ...
    GBytes * message, gpointer user_data)
{
  ...
  switch (type) {
    ...
    case SOUP_WEBSOCKET_DATA_TEXT:{
      gsize size;
      const gchar *data = (gchar const *)g_bytes_get_data (message, &size);
