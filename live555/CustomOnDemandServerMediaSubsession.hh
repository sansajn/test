...
class CustomOnDemandServerMediaSubsession: public ServerMediaSubsession {
protected: // we're a virtual base class
  CustomOnDemandServerMediaSubsession(UsageEnvironment& env, Boolean reuseFirstSource,
				portNumBits initialPortNum = 6970,
				Boolean multiplexRTCPWithRTP = False);
  virtual ~CustomOnDemandServerMediaSubsession();
...
private:
  ...
  friend class CustomStreamState;
};

class CustomDestinations {
public:
  CustomDestinations(struct in_addr const& destAddr,
               Port const& rtpDestPort,
               Port const& rtcpDestPort)
    : isTCP(False), addr(destAddr), rtpPort(rtpDestPort), rtcpPort(rtcpDestPort) {
  }
  CustomDestinations(int tcpSockNum, unsigned char rtpChanId, unsigned char rtcpChanId)
...
class CustomStreamState {
public:
  CustomStreamState(CustomOnDemandServerMediaSubsession& master,
              Port const& serverRTPPort, Port const& serverRTCPPort,
	      RTPSink* rtpSink,
	      unsigned totalBW, FramedSource* mediaSource,
	      Groupsock* rtpGS);
  virtual ~CustomStreamState();

  void startPlaying(CustomDestinations* CustomDestinations, unsigned clientSessionId,
  ...
  void sendRTCPAppPacket(u_int8_t subtype, char const* name,
			 u_int8_t* appDependentData, unsigned appDependentDataSize);
  void endPlaying(CustomDestinations* CustomDestinations, unsigned clientSessionId);
  void reclaim();
  ...
  RTCPInstance* rtcpInstance() const { return nullptr; }
...
private:
  CustomOnDemandServerMediaSubsession& fMaster;
