/*! stream dvoch jpeg obrazkou po 10 polickach, pre kazdy obrazok sa vytvara novy
source (pouzi openRTSP k zachiteniu vysledku) */
#include <vector>
#include <string>
#include <chrono>
#include <algorithm>
...
public:
	static jpeg_video_source_impl * createNew(UsageEnvironment & env,
		uint8_t * data, size_t size, std::chrono::microseconds time_per_frame, string const & name);
...
protected:
	jpeg_video_source_impl(UsageEnvironment & env, uint8_t * data, size_t size,
...
vector<uint8_t> source_data[2];  // two data sources
...
//	rtcp_sock.multicastSendOnly();
	...
	// source
	size_t len = 0;
	uint8_t * data = read_file(assets[0].string(), len);
	source_data[0].assign(data, data + len);

	data = read_file(assets[1].string(), len);
	source_data[1].assign(data, data + len);

	// wait 3s before start
...
{
	unsigned fps = 2;
	vector<uint8_t> & data = source_data[source_idx];
	source = jpeg_video_source_impl::createNew(*env, &data[0], data.size(),
		std::chrono::microseconds{1000000/fps}, string{"source_"} + to_string(source_idx));
...
void after_playing(void *)
{
	...
	Medium::close(source);
	source = nullptr;
	source_idx = (source_idx+1) % assets.size();
//	play();
}
...

{
	try
	{
		return new jpeg_video_source_impl(env, data, size, time_per_frame, name);
	}
...
jpeg_video_source_impl::jpeg_video_source_impl(UsageEnvironment & env,
	uint8_t * data, size_t size, std::chrono::microseconds time_per_frame,
	string const & name)
		: JPEGVideoSource{env}
...
{
	// vzdy odosle 1 policok a potom skonci
	if (_frame_count >= 1)
	{
		stopGettingFrames();
		handleClosure();
	...
	fFrameSize = jpeg_to_rtp(fTo, _jpeg_data, _jpeg_datalen);
