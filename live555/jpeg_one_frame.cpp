/*! stream jedneho policka jpeg obrazku (pouzi openRTSP k zachiteniu vysledku) */
#include <vector>
#include <string>
#include <chrono>
#include <algorithm>
...
public:
	static jpeg_video_source_impl * createNew(UsageEnvironment & env,
		string const & jpeg_file, std::chrono::microseconds time_per_frame, string const & name);
...
{
	unsigned fps = 2;
	source = jpeg_video_source_impl::createNew(*env, assets[0].string(),
	...
	string const & jpeg_file, std::chrono::microseconds time_per_frame, string const & name)
{
	try
	{
		return new jpeg_video_source_impl(env, jpeg_file, time_per_frame, name);
	}
...
jpeg_video_source_impl::jpeg_video_source_impl(UsageEnvironment & env,
	string const & jpeg_file, std::chrono::microseconds time_per_frame,
	string const & name)
		: JPEGVideoSource{env}
