...
void afterPlaying(void * clientData) {
  unsigned * trackNumber = static_cast<unsigned *>(clientData);

  BOOST_LOG_TRIVIAL(trace) << "...done reading from file (track-number:" << *trackNumber << ")";
...
void play() {
  BOOST_LOG_TRIVIAL(trace) << "Beginning to read from file...";

  // Start playing each track's RTP sink from its corresponding source:
  for (unsigned i = 0; i < 3; ++i) {
    if (trackState[i].sink != NULL && trackState[i].source != NULL) {
		trackState[i].sink->startPlaying(*trackState[i].source, afterPlaying, &trackState[i].trackNumber);
    }
  }
}
