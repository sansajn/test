/*! stream niekolkych jpeg obrazkou */
#include <vector>
#include <string>
#include <chrono>
#include <algorithm>
...
vector<fs::path> assets {
	"assets/girl_on_road.jpg",
	"assets/girl_face.jpg",
	"assets/girl_red.jpg",
	"assets/girl_with_glasses.jpg",
	"assets/road.jpg"
};
...
public:
	static jpeg_video_source_impl * createNew(UsageEnvironment & env,
		...
		std::chrono::microseconds time_per_frame);
...
protected:
	jpeg_video_source_impl(UsageEnvironment & env, string const & jpeg_file,
		std::chrono::milliseconds length, std::chrono::microseconds time_per_frame);
...
int main(int argc, char * argv[])
{
	...
	OutPacketBuffer::increaseMaxSizeTo(1000000);  // ask for bigger buffer
...
{
	unsigned fps = 2;
	source = jpeg_video_source_impl::createNew(*env, assets[current_asset_idx].string(),
		std::chrono::milliseconds{5000},	std::chrono::microseconds{1000000/fps});
	sink->startPlaying(*source, after_playing, nullptr);

	current_asset_idx = (current_asset_idx+1) % assets.size();
}
...
jpeg_video_source_impl * jpeg_video_source_impl::createNew(UsageEnvironment & env,
	string const & jpeg_file, std::chrono::milliseconds length, std::chrono::microseconds time_per_frame)
{
	try
	{
		return new jpeg_video_source_impl(env, jpeg_file, length, time_per_frame);
	}
		...
		, _jpeg_datalen{0}
{
	...
	BOOST_LOG_TRIVIAL(trace) << "jpeg_video_source_impl()";
...
void jpeg_video_source_impl::doGetNextFrame()
{
	...
	{
		BOOST_LOG_TRIVIAL(trace) << "doGetNextFrame(): end";
		handleClosure();
		return;
	}
	...
	BOOST_LOG_TRIVIAL(trace) << "doGetNextFrame(): " << _frame_count
