...
int main(int argc, char * argv[])
{
	...
	agg::conv_transform<
		agg::conv_curve<agg::path_storage>, agg::trans_affine> trans_curve{curve, T};

	agg::conv_stroke<agg::conv_curve<agg::path_storage>> stroke{curve};
	agg::conv_transform<
		agg::conv_stroke<agg::conv_curve<agg::path_storage>>, agg::trans_affine> trans_stroke{stroke, T};
	stroke.width(6.0);
	...
	//-----------------------
	write_ppm(buffer, frame_width, frame_height, "result6.ppm");
	delete [] buffer;
	return 0;
}
