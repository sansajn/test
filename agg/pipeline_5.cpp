...
int main(int argc, char * argv[])
{
	...
	T *= agg::trans_affine_scaling(0.4, 0.75);
	T *= agg::trans_affine_translation(frame_width/2, frame_height/2);

	using transform_type = agg::conv_transform<agg::path_storage, agg::trans_affine>;
	transform_type trans{path, T};

	agg::conv_curve<transform_type> curve{trans};
	agg::conv_stroke<agg::conv_curve<transform_type>> stroke{curve};
	stroke.width(6.0);
	...
	//-----------------------
	write_ppm(buffer, frame_width, frame_height, "result5.ppm");
	delete [] buffer;
	return 0;
}
